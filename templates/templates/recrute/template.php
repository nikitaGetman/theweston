...
<div id="rec" class="popup_header_callback call-back__modal popup-small popup__img">
	<div class="fancy">
		...
		<div class="fancy-inner">
			<h2 class="_H-h2-left js-hide">Запись на занятие</h2>
			<div class="modal-order__visible">
				<span class="_primary-text-left modal-span__bold">Какой язык будем постигать?</span>
				<form class="shake_btn_parsley_form" data-parsley-validate="" id="<?=$obName?>">
					<div class="modal-rb__inputs">
						<?
						$i = 0;
						$half = floor(count($arResult["ITEMS"]) / 2);
						?>
						<div class="modal-rb__inputs--first">
							<?foreach ($arResult["ITEMS"] as $key => $arElement):?>
								<label class="rb-input">
									<input
										class="rb-input__radio"
										type="radio"
										checked=""
										name="COUSE"
										data-parsley-multiple="COUSE"
										value="<?=$arElement["NAME"]?>"
									/>
									<span class="rb-input__check"></span>
									<span class="rb-image">
										<img src="<?=$arElement["PREVIEW_PICTURE"]["src"]?>" alt=""/>
									</span>
									<span class="rb-input__label">
										<?=$arElement["NAME"]?>
									</span>
								</label>
								<? if ($i++ == $half): ?>
									</div>
									<div class="modal-rb__inputs--second">
								<? endif; ?>
							<?endforeach;?>
						</div>
					</div>
